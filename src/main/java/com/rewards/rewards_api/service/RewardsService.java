 ...
 * Responsibilities:
 * <ul>
 * <li>Load transactions from the
 * {@link com.rewards.rewards_api.repository.TransactionRepository}.</li>
 * <li>Compute reward points according to business rules and aggregate them per
 * customer into time buckets (0-30, 31-60 and 61-90 days).</li>
 * <li>Return {@link com.rewards.rewards_api.dto.RewardResponse} DTOs for
 * consumption by the web layer.</li>
 * </ul>
 *
 * <p>
 * The service encapsulates business rules and keeps the controller thin. It
 * throws domain-specific exceptions (for example
 * {@link com.rewards.rewards_api.exception.TransactionNotFoundException}) which
 * are translated to HTTP responses by the global exception handler.
 *
 * @author Rewards
 * @since 0.0.1
 */
...
public class RewardsService {
	 ...
	 *
	 * <p>
	 * This method queries the {@link TransactionRepository} for all transactions
	 * and aggregates points per customer into three time buckets: 0-30 days, 31-60
	 * days and 61-90 days (relative to {@link java.time.LocalDate#now()}). Points
	 * are computed using {@link #calculatePoints(double)} and summed per bucket.
	 * Transactions older than 90 days are ignored.
	 *
	 * <p>
	 * If no transactions are available this method throws
	 * {@link com.rewards.rewards_api.exception.TransactionNotFoundException}.
	 *
	 * @return a list of {@link RewardResponse} with per-customer aggregated points
	 * @throws com.rewards.rewards_api.exception.TransactionNotFoundException when
	 *                                                                        repository
	 *                                                                        contains
	 *                                                                        no
	 *                                                                        transactions
	...
	/**
	 * Calculate reward points for a single transaction amount using the business
	 * rules:
	 * <ul>
	 * <li>No points for amounts <= 50.</li>
	 * <li>1 point per dollar for amount between 50 and 100 (exclusive of 50).</li>
	 * <li>2 points per dollar for amount above 100 plus the fixed 50 points for the
	 * 50-100 range.
	 * </ul>
	 *
	 * <p>
